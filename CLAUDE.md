...
- `sail-gpl.html` — a self‑contained marketing/landing page presenting the same license (single file: inline `<style>` in `:root` CSS variables + inline content; no external assets or build step). It paraphrases and restructures the markdown for the web; it is **not** a generated render of the `.md`.
- `DI_strategija_2iniciatyva_kibernetinis_saugumas.docx` — a related Lithuanian-language cybersecurity strategy document (binary; not part of the license text).

**Critical:** `SAIL-GPL-1.0.md` and `sail-gpl.html` are maintained **independently** — there is no pipeline that regenerates one from the other. When you change license substance, update **both** so they stay consistent (section numbers, tier table §8, the §7.x obligations, the comparison table §13, and the `srm/` bundle layout §14.2).
...
- **Sibling/companion specs:** QGPL (licenses model *outputs*) and OpenMDW (permissive base grant) are referenced as designed‑to‑interoperate. Keep claims about them consistent with §2 and the comparison table.

## Referenced (not vendored) tooling

The license describes a reference CLI `sail` (`sail sign | verify | audit`) and an `srm/` bundle layout (§14). **None of this exists in this repo** — it is specified prose, not implemented code. If asked to implement the toolkit, treat §7 and §14 as the spec.
